// run-underwrite-source sends UNDERWRITE/COMPUTE_LIMIT_FROM_SOURCE through Flare's data
...
func main() {
	...
	logger.Infof("Sending UNDERWRITE/COMPUTE_LIMIT_FROM_SOURCE (%d bytes; no key, no revenue in calldata)...", len(payload))
	tx, err := sender.SendComputeLimitFromSource(opts, payload)
	if err != nil {
		if reason := fccutils.DecodeRevertReason(err); reason != "" {
			fccutils.FatalWithCause(errors.Errorf("sendComputeLimitFromSource: %s (revert: %s)", err, reason))
		}
		fccutils.FatalWithCause(errors.Errorf("sendComputeLimitFromSource: %s", err))
	}
	...
	logger.Infof("Done. The enclave read the processor itself: no key, no figure, ever on chain.")
}
